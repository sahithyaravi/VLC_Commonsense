        ...
        if relation in relation_map:
            ...
            for beam in beams:
                if beam != " none" and beam != "   ":
                    ...
                    context.append(sent.captitalize())
    return context, top_context


def expansions_to_sentences(expansions, sentences, srl=False, parallel=True):
    ...
    if parallel:
        ...
        contexts = []
        top_contexts = []
        with tqdm(total=len(expansions.keys())) as pbar:
            for key, exp in expansions.items():
                pbar.update(1)
                context, top_context = convert_job(sentences, key, exp, srl)
